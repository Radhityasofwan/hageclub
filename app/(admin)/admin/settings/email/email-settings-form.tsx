...
export function EmailSettingsForm({ settings, adminEmail }: Props) {
  ...
  async function handleSave() {
    ...
    const filtered = updates.filter((u) => u.value !== "••••••••");
    try {
  ...
  return (
    <div className="space-y-4">
          ...
          {orderedSettings.map((field) => {
            ...
            return (
              <div key={field.key} className="px-6 py-4">
                <div className="mb-2">
                  <label htmlFor={field.key} className="text-sm font-medium text-primary flex items-center gap-2">
                    {field.label}
                    {field.isSecret && (
                  ...
                  {field.hint && <p className="text-xs text-muted mt-0.5">{field.hint}</p>}
                </div>
                <div className="relative">
                  <input
                    ...
                    placeholder={isMasked ? "Biarkan kosong untuk tidak mengubah" : (PLACEHOLDERS[field.key] ?? "Masukkan nilai...")}
                    autoComplete="off"
                    spellCheck={false}
                  ...
                  {field.isSecret && (
                    <button
                      type="button"
                      onClick={() => setRevealed((prev) => ({ ...prev, [field.key]: !isRevealed }))}
      ...
      <div className="rounded-sm border border-border bg-white p-4 text-xs text-muted space-y-1.5">
        ...
        <p>
          • <strong>cPanel Hosting:</strong> Gunakan mail server hosting kamu (contoh:{" "}
          <code className="bg-accent px-1 rounded-sm">mail.namadomain.com</code>), port 465 dengan SSL.
        </p>
        <p>
          • <strong>Gmail:</strong> Aktifkan 2FA lalu buat App Password di Google Account. Port 587, SSL:{" "}
          <code className="bg-accent px-1 rounded-sm">false</code>.
        </p>
        <p>
          • Konfigurasi di-cache selama 5 menit. Setelah simpan, tunggu 5 menit atau restart server agar perubahan aktif.
        </p>
      </div>
    </div>
  );
