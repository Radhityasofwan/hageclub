...
async function triggerBuild(buildSettings) {
  ...
  const buildData = {
    ...(buildSettings || {}),
    node_version: 22,
    build_command: 'npx prisma generate && npx prisma migrate deploy && npm run build && cp -r .next/static .next/standalone/.next/static && cp -r public .next/standalone/public',
